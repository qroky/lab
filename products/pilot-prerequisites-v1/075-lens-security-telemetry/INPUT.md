---
atom: ATOM-075
product: Lens verdict — security-telemetry (the fan of ATOM-007, SYNTHESIS-PROTOCOL LF1-LF7)
...
# ATOM-075 — Input Specification (lens atom)

## 1. Identity
| Field | Value |
| :---- | :---- |
| Lens | **security-telemetry** |
| Executor role | security-telemetry-reviewer (`roles/security-telemetry-reviewer.md` — read it first, it IS you) |
| Atom ID | ATOM-075 |
...
Adversarial read: enumerate every path data can leave the founder's machine given these exact scripts; is the whitelist deny-by-default in CODE, not just in intent (read push.sh line by line); can the negative test be fooled; does consent promise EXACTLY what the scripts do - list every promise and its enforcing line; deletion date and OFF-switch: mechanism or prose; does the render script leak business content into showcase artifacts; does bootstrap.sh touch anything outside the founder's repo or phone home.

## 5. Deliverable
`075-lens-security-telemetry/LENS-VERDICT.md`:
