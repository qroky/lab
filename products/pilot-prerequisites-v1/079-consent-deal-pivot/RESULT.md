...
| `## O alegere separată...` / `## Отдельный выбор...` | Yes | Same optional item: one plain sentence about what was built, to партнёр-акселератор (MD), off by default, independently revocable |
...
| 3 | Reads the separate opt-in item (accelerator partner (MD) sentence) | 131 | 0.9 min |
