...
# ROLE-008 — Input Specification

## 1. Product Identity
| Field | Value |
| :---- | :---- |
| Product | `roles/ops-capacity-reviewer.md` — role spec, ~40 lines |
| Atom ID | ROLE-008 |
...
| Consumer(s) | ATOM-076 (lens: owner operational capacity) |
| Verification mode | self (ROLE-001 precedent: hard-criteria self-check is acceptance at draft) |
| Budget envelope | ~12k tokens |

## 2. Job To Be Done
Create the role **Ops Capacity Reviewer** so its consuming atoms can run under a professional
optic that is specific, not generic. The role does workload arithmetic for the pilot owner (one human): daily review load at >=2 concurrent founders, SLA feasibility (reaction to blocked within one business day), intervention-approval overhead per Flow Support v0 rules, calendar collision with the owner's other duties. It answers the shared fan question with hours-per-week numbers and breaking points, not reassurance.

## 3. Definition of Done
**Hard criteria:**
- H1. `roles/ops-capacity-reviewer.md` exists, 30–50 lines, follows the venture-strategist section set; frontmatter `description` states in one line which atoms need this role. Check: file + wc -l.
...
- H4. Provenance table: Created / ROLE-008 / 2026-07-07. Check: grep.
