...
> Runtime instruction for this session. Execute **one atom**, per the constitution (`framework/ATOM-SPEC.md`, `framework/FEV-PROTOCOL.md`, `framework/REPO-STRUCTURE.md`, `framework/ORCHESTRATION.md`) and the binding (`runtime/claude/README.md`).
> Step 0 — materialize Formulate outputs per RS/SM rules. Execute; run blind Verify per FEV §2 (envelope: read estimate ×3.5 **+ 40k constant term** — per the small-package lesson in the carry-over of ATOM-004, the runtime's per-turn overhead dominates small packages); request human sign-off; close per L7/RB6.
...
---
*End of launch file. The framework introduces itself.*
